...
export function GrowBoxDashboard() {
  ...
  const setFanSpeed = (box: "vegie" | "bloom", speed: number) => {
    if (!data) return
    setData({ ...data, [box]: { ...data[box], fanSpeed: speed } })
  }

  const setTargetHumidity = (box: "vegie" | "bloom", humidity: number) => {
    if (!data) return
    setData({ ...data, [box]: { ...data[box], targetHumidity: humidity } })
  }

  const setBloomDate = (box: "vegie" | "bloom", date: Date) => {
    if (!data) return
    setData({ ...data, [box]: { ...data[box], bloomStartDate: date } })
  }

  const setHarvestDate = (box: "vegie" | "bloom", date: Date) => {
    if (!data) return
    setData({ ...data, [box]: { ...data[box], harvestDate: date } })
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <footer className="py-2 text-center text-[9px] text-muted-foreground">
        {lastUpdated
          ? `Live · ${lastUpdated.toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`
          : "Verbinde…"}
      </footer>
    </div>
  )
